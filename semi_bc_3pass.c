...
#define RADIX_NLEN 2
#define M 3 // Number of window to check per iteration
...
mzd_t* semi_bc(mzd_t* G, uint64_t time_sec, uint64_t l, uint64_t c) {
    ...
    uint64_t p = 2, iter = 0, mwin = 0, delta = 0, citer = 0, total = 0, done = 0, nb_total_cw = 0;
    ...
    while (1) {
        ...
        * So we sort em all. */
        ...
        for (mwin = 0; mwin < M; mwin++) {
            ...
            lc_tab_alias_second_sorted[mwin] = radixsort(lc_tab_second[mwin], lc_tab_second_sorted[mwin], lc_tab_second_size[mwin], aux);
            lc_tab_alias_third_sorted[mwin] = radixsort(lc_tab_third[mwin], lc_tab_third_sorted[mwin],lc_tab_third_size[mwin], aux);
        }
