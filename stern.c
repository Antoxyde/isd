...
mzd_t* isd_stern_canteaut_chabaud_p2(mzd_t* G, uint64_t niter, uint64_t sigma) {
    ...
    /*
        uint64_t max = 0;
        for (uint64_t i = 0; i < (1 << sigma); i++) {
            bucket* b = table_retrieve_bucket(tab,i);
            if (b && b->count > max) {
                max = b->count;
            }
        }
        printf("MAx bucket size : %ld\n", max);
*/
...
mzd_t* isd_stern_canteaut_chabaud_p3(mzd_t* G, uint64_t niter, uint64_t sigma) {
    ...
    uint64_t p = 3, iter, nb_collision = 0;
    rci_t n = G->ncols, comb1[p], comb2[p], min_comb[2*p],lambda, mu, tmp, i, j;
    rci_t* comb3;
